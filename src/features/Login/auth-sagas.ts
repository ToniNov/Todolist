...
import {authAPI, LogInOutResponseType, LoginParamsType, ResponseType} from "../../api/todolists-api";
...
export function* loginWorkerSaga(action: ReturnType<typeof loginSagaAC>) {
    ...
    try {
        const data: LogInOutResponseType = yield call(authAPI.login, action.data)
...
export function* logoutWorkerSaga(action: ReturnType<typeof logoutSagaAC>) {
    yield put(setAppStatusAC('loading'))
    try {
        const data: LogInOutResponseType = yield call(authAPI.logout)
        if (data.resultCode === 0) {
